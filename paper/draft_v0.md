...
A three-channel variant adding an *action-phrase* channel `ŝ(a,F)` (weights `w_q,w_o,w_a`) is evaluated separately as E1 in §6.6; combining it with Stage A is future work (no factorial run yet).
...
*Implementation note (MVP).* The runs above use CLIP 1 fps frame scoring with z-normalized object conditioning and temporal smoothing for selection, Qwen3-VL for grounding, and SDXL-Turbo (V-STaR) for generation; the design components BGE-M3 pruning, recursive AKS coverage, and shared-RAG feedback (§4) are **not fully enabled** in this run. Numbers span V-STaR 150 and HC-STVG 200–1000 (per-section n as stated); the bridged external baseline is in §6.6 and the FLUX-Kontext faithfulness study in §6.7.
...
**Stage-A grid density [VERIFIED, n = 300].** Densifying the sparse grid monotonically improves the window and the end metric — `g=10/15/20` → temporal recall 0.717/0.740/0.763, acc@0.3 **0.570/0.600/0.613** — shrinking the oracle gap to ≈0.17 at `g=20` (cost: proportionally more image tokens in the single Stage-A call). Saturation and cost-accuracy trade-off beyond `g=20` are TODO.
...
**The faithfulness–creativity trade-off (measured).** We added a second human-rated axis — *poster-likeness* (1 = looks like a screenshot, 2 = some polish, 3 = looks like a real poster) — and rated three instruction tiers on the same 10 keyframes (30 pairs, shuffled blind):

| Instruction tier | Human faithful | Poster-likeness (1–3) |
|---|---|---|
| A: permissive ("cinematic poster, dramatic lighting") | 1/10 (10%) | **2.50** |
...
Tier C confirms that **locking content preserves faithfulness (90%)**, but freeing only the lighting/grading vocabulary did *not* recover artistry (1.20 ≈ B's 1.10): under a single instruction, FLUX-Kontext appears to trade content fidelity and stylistic boldness against each other rather than composing them. A maximal-artistry constrained variant is the one remaining scheduled probe; if it also fails, the finding stands as a **negative result for single-instruction control**, motivating dual-channel conditioning (identity/layout locked by an image condition, style freed by text — e.g., IP-Adapter/ControlNet-style, already part of our method design §4.3④) as the path to the application's sweet spot (§7).
...
**Limitations (honest).**
- **Temporal keyframe selection was the dominant bottleneck — partially addressed, gap remains.** *Before the two-stage selector*, temporal recall was **0.26 (V-STaR) / 0.52–0.56 (HC-STVG)**, capping joint localization at **0.23 / 0.42–0.43** even though the box is accurate once the moment is right (IoU 0.72 / 0.66). The two-stage selector (§4.3②, §6.6) lifts HC-STVG temporal recall to **0.720** and joint acc@0.3 to **0.567** (282-clip subset), but an oracle gap of **≈0.21** remains, and Stage A adds one MLLM call per clip (+1.46 s, measured §6.7). Future work: g>20 saturation sweep, iterative window refinement, audio/subtitle cues, a learned selector; the two-stage result is not yet re-verified on V-STaR. **[VERIFIED]**
- **Grounding & generation.** Qwen3-VL native grounding ≥ Grounding-DINO-base on our data (V-STaR joint acc@0.3 0.28 vs. 0.24); stronger detectors (G-DINO-large / DINO-X, API-gated) are future work. The end-to-end V-STaR runs used SDXL-Turbo; the faithfulness study (§6.7) already uses the full FLUX-Kontext generator.
- **Scale, data provenance & generator.** Numbers are on dev subsets (V-STaR 150 / HC-STVG 200–1000; selector comparison on a 282-clip common subset) with SDXL-Turbo for the end-to-end runs (FLUX-Kontext used in the §6.7 faithfulness study), **not the full splits**; the only external baseline run so far is the bridged NeurIPS'25 zero-shot STVG (§6.6), whose full DSTH mode could not be reproduced from the official code (issue filed). HC-STVG videos are from a *community HF mirror* (`ShijianW01/hc-stvg2`), not the official package — we verified its annotation schema and gold tubes match the official format, but a re-run on the official release is on the list. The grounding-target noun phrase is a *heuristic* extractor (verb-boundary + `sub` fallback), over-long in ~28% of HC-STVG samples, which conservatively lowers the reported numbers. Scaling to full splits (HC-STVG 1901 / VKIG-Bench 300+→1–2k), the full FLUX-Kontext generator, proper metrics (VQAScore/DreamSim), and baselines are TODO.
