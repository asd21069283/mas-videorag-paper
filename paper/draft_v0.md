...
- **The faithfulness–creativity trade-off is measured but unresolved.** Under single-instruction control, locking content preserves faithfulness (90–100%) while poster-likeness rises only partway as artistic instruction is pushed (B 1.10 → C 1.20 → D 1.50, §6.7) and plateaus well below the 2.50 reached when content is unlocked. This bounded negative result (n=10, one annotator) motivates but does not yet validate **dual-channel conditioning** (image condition locks identity/layout, text frees style, §4.3④) — the designed path to the sweet spot remains unverified.
