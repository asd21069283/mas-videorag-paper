...
**③ Object-level grounding.** For each `(F∈I*, o∈O_Q)`, open-vocabulary detection yields `(box, ρ(o,F))`, producing evidence set `B = {(F,o,box,ρ)}`. **In all verified runs this role is filled by Qwen3-VL's native grounding** (a bbox prompt on the selected keyframe; §6.2–6.3); Grounding-DINO and Video-RAG's APE [arXiv:2411.13093] are the alternatives evaluated on 50 clips and dropped in favor of Qwen3-VL (§6.2, §6.3). `ρ` is the detector's returned confidence where available.
